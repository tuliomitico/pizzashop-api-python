...
month_order_blp = Blueprint("month_orders", "month_orders", url_prefix="/metrics/month-orders-amount", description="Operations on dashboard")

@month_order_blp.route("",methods=['GET'])
...
def index():
  ...
  orders_per_month: list[MonthOrderSchema] = Orders.query.with_entities(
  ...
  sql_statement = orders_per_month.statement.compile(engine, compile_kwargs={"literal_binds": True}).string
  print(sql_statement)
  orders = MonthOrderSchema().dump(orders_per_month,many=True)
  print(orders)
